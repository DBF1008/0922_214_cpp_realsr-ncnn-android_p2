...
public class DirectoryProcessActivity extends AppCompatActivity {
    ...
    private boolean isSettingFromActivity = false;
    ...
    private void updateAutoOutputPath(String inputPath) {
        if (cbAutoOutput.isChecked()) {
            // Use SAF URI to extract directory name when available (more reliable
            // for non-primary storage / SD cards where filesystem path may not resolve).
            String dirName;
            if (inputDirUri != null) {
                dirName = SafPathHelper.getDisplayNameFromTreeUri(inputDirUri);
            } else {
    ...
    private void selectDirectory(int requestCode) {
        ...
        // Use the previously selected tree URI to initialize the picker position.
        // A tree URI from ACTION_OPEN_DOCUMENT_TREE is accepted by EXTRA_INITIAL_URI;
        // a file:// URI is NOT and was silently ignored on many devices.
        if (requestCode == REQUEST_CODE_INPUT_DIR && inputDirUri != null) {
            intent.putExtra(DocumentsContract.EXTRA_INITIAL_URI, inputDirUri);
        } else if (requestCode == REQUEST_CODE_OUTPUT_DIR && outputDirUri != null) {
    ...
    /**
     * @deprecated Use {@link SafPathHelper#getAbsolutePathFromTreeUri(Uri)} directly.
     */
    private String getAbsolutePathFromTreeUri(Uri treeUri) {
        return SafPathHelper.getAbsolutePathFromTreeUri(treeUri);
    }
